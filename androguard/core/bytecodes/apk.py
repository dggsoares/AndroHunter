...
class APK:
    ...
    #Alteracao AndroHunter
    def get_intents(self) :
        """
            Return the android:name attribute of all intents

            :rtype: a list of string
        """
        list_action = self.get_elements("action", "android:name")
        list_category = self.get_elements("category", "android:name")
        list_service = self.get_elements("service", "android:name")
        list_intents = []

        for action in list_action:
            if action not in list_service:
                list_intents.append(action)

        for category in list_category:
            if category[category.find('category')+9:].isupper():
                if category not in list_intents:
                    list_intents.append(category)
