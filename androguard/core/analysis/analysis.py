...
class TaintedVariable :
    ...
    def show_paths_Androhunter(self, vm):
        return show_PathVariable_Androhunter(vm, self.get_paths())
...
def show_Path_Androhunter(vm, path):
    ...
    if isinstance(path, PathVar) :
        ...
        return ("Info var: %s --> calls --> Class: %s Method: %s" % (info_var,
                                            dst_class_name,
                                            dst_method_name))
    else :
        if path.get_access_flag() == TAINTED_PACKAGE_CALL :
            ...
            dst_class_name, dst_method_name, dst_descriptor =  path.get_dst( cm )
            return ("Class: %s Method: %s --> calls --> Class: %s Method: %s" % (src_class_name,
                                                        src_method_name,
                                                        dst_class_name,
                                                        dst_method_name))
        else :
            src_class_name, src_method_name, src_descriptor =  path.get_src( cm )
            return ("Class: %s Method: %s" % (src_class_name,
                                        src_method_name))
...
#Method for Androhunter
def show_PathVariable_Androhunter(vm, paths) :
    data = []
    for path in paths :
      access, idx = path[0]
      m_idx = path[1]
      method = vm.get_cm_method( m_idx )
      str = ("%s->%s" % (method[0], method[1]))
      if str not in data:
        data.append(str)
    return data
